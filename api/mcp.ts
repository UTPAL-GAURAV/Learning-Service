...
function buildServer(userId: string): McpServer {
  ...
  server.tool(
    "get_session",
    "Get full session data for a topic. Returns null if session doesn't exist.",
    { topic_slug: z.string() },
    async ({ topic_slug }) => {
  ...
  server.tool(
    "update_session",
    "Partially update a session: notes, readiness score, covered/pending topics, key concepts",
    {
      topic_slug: z.string(),
      patch: z.object({
  ...
  server.tool(
    "add_qa_card",
    "Add a Q&A card to a topic session",
    {
      topic_slug: z.string(),
      card: z.object({
        id: z.string(),
  ...
  server.tool(
    "update_qa_attempts",
    "Record an attempt on a Q&A card (appends to attempts array, tracks wrong count)",
    {
      card_id: z.string(),
      attempt: z.object({
        timestamp: z.string(),
  ...
  server.tool(
    "record_score",
    "Record a readiness score for a topic",
    {
      topic_slug: z.string(),
  ...
  server.tool(
    "get_score_history",
    "Get score history for one or all topics",
  ...
  server.tool(
    "get_weak_areas",
    "Get weak areas for one or all topics",
    { topic_slug: z.string().optional() },
    async ({ topic_slug }) => {
  ...
  server.tool(
    "upsert_weak_area",
    "Insert or update a weak area for a topic",
    {
      topic_slug: z.string(),
      sub_topic: z.string(),
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const transport = new StreamableHTTPServerTransport({
    sessionIdGenerator: undefined, // stateless — no session resumption needed
  });
