...
async function startMcpServer(token) {
  ...
  server.tool("get_user_context", "Get user profile and active sessions summary", {}, async () => {
    ...
    return { content: [{ type: "text", text: JSON.stringify({ user, sessions }) }] };
  });
  ...
  server.tool("create_session", "Create a new learning session", {
    ...
    const data = await apiCall("POST", "/api/sessions", token, {
      topicSlug: topic_slug, topicName: topic_name, syllabusTopics: syllabus_topics,
    });
    return { content: [{ type: "text", text: JSON.stringify(data) }] };
  });
  ...
  server.tool("get_score_history", "Get score history for one or all topics", {
    ...
    const data = topic_slug
      ? await apiCall("GET", `/api/sessions/${topic_slug}/scores`, token)
      : await apiCall("GET", "/api/sessions", token);
    return { content: [{ type: "text", text: JSON.stringify(data) }] };
  });
  ...
  server.tool("get_weak_areas", "Get weak areas for one or all topics", {
    ...
    const p = topic_slug ? `/api/weak-areas?topic=${topic_slug}` : "/api/weak-areas";
    const data = await apiCall("GET", p, token);
    return { content: [{ type: "text", text: JSON.stringify(data) }] };
  });
